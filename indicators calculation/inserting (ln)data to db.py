...
adj_low = np.array(df['low'])
adj_high = np.array(df['high'])
adj_close = np.array(df['close'])
adj_final = np.array(df['final'])

adj_low = np.log(adj_low)
adj_high = np.log(adj_high)
adj_close = np.log(adj_close)
adj_final = np.log(adj_final)
...
for i in range(6, 21):
    df['SMA{}'.format(i)] = ((talib.SMA(np.array(adj_close), i) / adj_close) + 1) / 2
for i in range(6, 21):
    df['EMA{}'.format(i)] = ((talib.EMA(np.array(adj_close), i) / adj_close) + 1) / 2
for i in range(6, 21):
    df['WMA{}'.format(i)] = ((talib.WMA(np.array(adj_close), i) / adj_close) + 1) / 2
for i in range(6, 21):
    df['HMA{}'.format(i)] = ((talib.WMA(2 * talib.WMA(np.array(adj_close), i) - talib.WMA(np.array(adj_close), 2 * i),math.sqrt(2 * i)) / adj_close) + 1) / 2
for i in range(6, 21):
    df['TEMA{}'.format(i)] = ((talib.TEMA(np.array(adj_close)) / adj_close) + 1) / 2
for i in range(6, 21):
...
for i in range(6, 21):
    df['MACD{}'.format(i)] = talib.MACD(np.array(adj_close), fastperiod=i, slowperiod=2 * i + 2)[2] + 1 / 2
for i in range(6, 21):
    ppo = talib.PPO(np.array(adj_close), i, 2 * i + 2)
    df['PPO{}'.format(i)] = (ppo - talib.EMA(ppo, 9) / 10) + 1 / 2
for i in range(6, 21):
    df['ROC{}'.format(i)] = (talib.ROC(np.array(adj_close), i) + 50) / 150
for i in range(6, 21):
...
query = ("INSERT INTO ln_indicators"
